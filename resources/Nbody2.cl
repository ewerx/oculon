...
float3 gravitation(float4 myPos, float3 accel, float softeningSquared, __local float4* sharedPos)
{
...
float3 computeBodyAccel_MT(float4 bodyPos, 
...
float3 computeBodyAccel_noMT(float4 bodyPos, 
                             __global float4* positions, 
                             int numBodies, 
                             float softeningSquared, 
                             __local float4* sharedPos)
            ...
            __local float4* sharedPos)
{
    ...
    unsigned int index = mul24(blockIdxx, blockDimx) + threadIdxx;
    float4 pos = oldPos[index];   
    float3 accel = computeBodyAccel_noMT(pos, oldPos, numBodies, softeningSquared, sharedPos);
