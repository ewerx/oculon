...
float GetSceneDistanceMain( out vec4 vOutUVW_Id, const in vec3 vPos )
{
	...
	{
		fOutDist = fWallDist;
		vOutUVW_Id = vec4(atan(vPos.x, vPos.y) * 2.0 / (3.14159 * 2.0), vPos.z * 0.05, 0.0, 1.0);
		
		fNoiseMag = 0.1;
	}
    
	// noise
	float fSample = texture2D(iChannel0, vOutUVW_Id.xy).r;
...
void GetSurfaceInfo(out vec3 vOutAlbedo, out vec3 vEmissive, const in C_Intersection intersection )
{
	...
	if(intersection.fObjectId == 1.0)
	{
		...
		// sample a lower mip for subsurface effect
		vec3 vSample = texture2D(iChannel0, vUV.xy, 4.0).rgb;
