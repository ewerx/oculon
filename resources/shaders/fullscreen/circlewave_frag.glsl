...
//         float sound = mix( texture2D( iChannel0, vec2((u+.5)/256.0,1.0) ).x, texture2D( iChannel0, vec2((u+1.5)/256.0,1.0) ).x, f );
		float sound = texture2D( iChannel1, vec2(uv.x,1.0) ).x;
    ...
    if (iBackgroundFlash)
    {
        col = vec4(0);
        col.w = 1.0;
        col.z  += texture2D( iChannel1, vec2(.000,.25) ).x;
        col.zy += texture2D( iChannel1, vec2(.125,.25) ).xx*vec2(1.5,.5);
        col.zy += texture2D( iChannel1, vec2(.250,.25) ).xx;
        col.zy += texture2D( iChannel1, vec2(.375,.25) ).xx*vec2(.5,1.5);
        col.y  += texture2D( iChannel1, vec2(.500,.25) ).x;
        col.yx += texture2D( iChannel1, vec2(.625,.25) ).xx*vec2(1.5,.5);
        col.yx += texture2D( iChannel1, vec2(.750,.25) ).xx;
        col.yx += texture2D( iChannel1, vec2(.875,.25) ).xx*vec2(.5,1.5);
        col.x  += texture2D( iChannel1, vec2(1.00,.25) ).x;
        col *= iColor2;
        // vignetting
