...
vec3 firePalette(float i){
    ...
    float maxTemp = 1400.0; /// param
    float minTemp = 1300.0; /// param
    float T = maxTemp + minTemp*i; // Temperature range (in Kelvin).
...
float map(vec3 p) {
    ...
    float bumpiness = 8.0;/// param
    p = sin(p*bumpiness + cos(p.yzx*8.));
...
float logBisectTrace( in vec3 ro, in vec3 rd){
    ...
    for (int i=0; i<80; i++){
        ...
        //if we crossed but didnt detect, use bisection method
        if (sign(d) != sgn){ break; }
