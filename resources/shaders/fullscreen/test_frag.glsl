...
//vec2 iChannelResolution = vec2(256.0,256.0);
...
vec3 raymarch( in vec3 ro, in vec3 rd, in vec2 ani )
{
    // background color
    ...
    // dithering
	float t = 0.03*texture2D( iChannel0, gl_FragCoord.xy/256.0 ).x;
...
void main(void)
{
	...
	// camera parameters
	vec4 camPars = texture2D( iChannel0, floor(1.0+iGlobalTime/5.5)*vec2(5.0,7.0)/vec2(256.0,256.0) );
