...
void Deformer::setup()
{
    ...
    // Load shaders
	try {
		mShaderTex = gl::GlslProg( loadResource( "terrain_tex_vert.glsl" ), loadResource( "terrain_tex_frag.glsl" ) );
	} catch ( gl::GlslProgCompileExc ex ) {
		console() << "Unable to compile texture shader:\n" << ex.what() << "\n";
	}
	try {
		mShaderVtf = gl::GlslProg( loadResource( "terrain_vtf_vert.glsl" ), loadResource( "terrain_vtf_frag.glsl" ) );
	...
	mMeshTexture = gl::Texture( loadImage( loadResource( "black_grid_2level.jpg" ) ) );
...
void Deformer::drawDebug()
{
    ...
    gl::draw( mVtfFbo.getTexture(0), preview );
